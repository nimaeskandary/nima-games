(ns nimaeskandary.vulcan-tutorial.chapter-2.eng.graph.vk.instance
  (:require [clojure.set :as set]
            [nimaeskandary.vulcan-tutorial.chapter-2.eng.graph.vk.vulkan-utils
             :as vulkan-utils]
            [nimaeskandary.vulcan-tutorial.chapter-2.eng.proto.instance :as
             proto.instance])
...
(defn get-instance-exts
  []
  (with-open [stack (MemoryStack/stackPush)]
    (let [^IntBuffer num-exts-buff (.callocInt stack 1)
          _ (VK13/vkEnumerateInstanceExtensionProperties ^String nil
                                                         num-exts-buff
                                                         nil)
          num-exts (.get num-exts-buff 0)
          _ (println "instance supports %d extensions" num-exts)
      ...
      (set (for [^Integer i (range num-exts)]
             (let [ext-name (-> props-buff
                                ...
                                .extensionNameString)]
               (println "supports extension %s" ext-name)
               ext-name))))))
