(ns nimaeskandary.super-kaolio.interface.start
  (:require [nimaeskandary.super-kaolio.interface.core :as c]
            ;; music disabled for now
            ;[super-koalio.music :as m]
            [play-cljc.gl.core :as pc])
  (:import [org.lwjgl.glfw GLFW Callbacks GLFWCursorPosCallbackI
            GLFWKeyCallbackI GLFWMouseButtonCallbackI GLFWCharCallbackI
            GLFWFramebufferSizeCallbackI GLFWWindowCloseCallbackI
            GLFWScrollCallbackI]
...
(defn on-mouse-click!
  ...
  (swap! c/*state assoc
    :mouse-button
    (when (= action GLFW/GLFW_PRESS) (mousecode->keyword button))))
...
(extend-type Window
 Events
   ...
   (on-char [{:keys [handle]} codepoint] (on-char! handle codepoint))
   (on-resize [{:keys [handle]} width height] (on-resize! handle width height))
   (on-scroll [{:keys [handle]} xoffset yoffset]
     (on-scroll! handle xoffset yoffset))
   (on-tick [this game] (c/tick game)))

(defn listen-for-events
  [{:keys [handle], :as window}]
  (doto handle
    (GLFW/glfwSetCursorPosCallback (reify
...
(defn ->window
  []
  (when-not (GLFW/glfwInit) (throw (Exception. "Unable to initialize GLFW")))
    ...
    (loop [game game]
      (when-not (GLFW/glfwWindowShouldClose handle)
        (let [ts (GLFW/glfwGetTime)
              game
              (assoc game :delta-time (- ts (:total-time game)) :total-time ts)
...
(defn -main
  [& args]
  (let [window (->window)] (start (pc/->game (:handle window)) window)))
