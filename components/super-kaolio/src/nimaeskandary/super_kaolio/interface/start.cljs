(ns nimaeskandary.super-kaolio.interface.start
  ...
  #_(:require-macros [super-koalio.music :refer [build-for-cljs]]))
...
(defn game-loop
  ...
  (let [game (c/tick game)]
    (js/requestAnimationFrame (fn [ts]
                                (let [ts (msec->sec ts)]
...
(defn keycode->keyword
  [keycode]
  (condp = keycode 37 :left 39 :right 38 :up nil))
...
(defn listen-for-resize
  [context]
  (events/listen js/window "resize" (fn [event] (resize context))))
...
;; build music, put it in the audio tag, and make the button toggle it on and
;; off
...
(set! (.-onclick button)
      (fn [e] (if (swap! play-music? not) (.play audio) (.pause audio))))
