(ns nimaeskandary.super-kaolio.interface.tiles
  (:require [nimaeskandary.super-kaolio.interface.utils :as utils]
            ...
            #?@(:clj [[clojure.java.io :as io] [tile-soup.core :as ts]])))

#?(:clj (defmacro read-tiled-map
          [fname]
          (-> (str "super-kaolio/public/" fname)
              io/resource
              slurp
              ts/parse
              pr-str)))
    ...
    (utils/get-image
     ...
     (fn [{:keys [data width height]}]
       (let [entity (e/->image-entity game data width height)
             ...
             images (vec (for [y (range tiles-vert)
                               x (range tiles-horiz)]
                           (t/crop entity
                                   (* x tilewidth)
                                   (* y tileheight)
...
(defn touching-tile
  [{:keys [layers], :as tiled-map} layer-name x y width height]
  (let [layer (get layers layer-name)
        start-x (int x)
        start-y (int y)
        end-x (int (+ x width))
