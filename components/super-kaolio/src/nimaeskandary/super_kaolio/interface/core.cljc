(ns nimaeskandary.super-kaolio.interface.core
  (:require [nimaeskandary.super-kaolio.interface.utils :as utils]
            [nimaeskandary.super-kaolio.interface.move :as move]
            [clojure.edn :as edn]
            ...
            #?(:clj [nimaeskandary.super-kaolio.interface.tiles :as tiles :refer
                     [read-tiled-map]]
               :cljs [nimaeskandary.super-kaolio.interface.tiles :as tiles
                      :refer-macros [read-tiled-map]])))

;; https://github.com/oakes/play-cljc-examples/tree/master/super-koalio
  ...
  (utils/get-image
   ...
   (fn [{:keys [data width height]}]
     (let [entity (c/compile game (e/->image-entity game data width height))
           images
           (vec (for [i (range 5)]
                  (t/crop entity (* i koala-width) 0 koala-width koala-height)))
...
(defn tick
  ...
  (let [state (swap! *state (fn [state]
                              (->> state
                                   ...
                                   (move/prevent-move)
                                   (move/animate game))))
        {:keys [pressed-keys player-x player-y player-width player-height
                direction player-images player-image-key tiled-map
                tiled-map-entity camera]}
        state
