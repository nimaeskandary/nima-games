(ns nimaeskandary.super-kaolio.interface.utils
...
(defn get-image
  [fname callback]
  #?(:clj (let [is (io/input-stream (io/resource (str "super-kaolio/public/"
                                                      fname)))
...
(defn get-size
  ...
  #?(:clj (let [*width (MemoryUtil/memAllocInt 1)
                *height (MemoryUtil/memAllocInt 1)
                _ (GLFW/glfwGetFramebufferSize ^long (:context game)
                                               *width
                                               *height)
                w (.get *width)
                h (.get *height)]
...
(defn get-width [game] (first (get-size game)))

(defn get-height [game] (second (get-size game)))
